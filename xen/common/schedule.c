...
static void schedule(void)
{
    ...
    if (!dom0) {
		...
		if (next->domain->domain_id == 0)
			dom0 = next->domain;

		// when we first get a ptr to dom0, init default values of shared_scheduler_info
		if (dom0) {
			for (int i = 0; i < 4; i++) {
		...
		for (int i = 0; i < 4; i++) {
			...
			if ((prev->domain->domain_id != DOMID_IDLE) && (prev->domain->domain_id != 0)) {
				if (dom0->shared_info->native.sched_infos[i].domid == prev->domain->domain_id) {
					...
					// do not set end time here, because we need to preserve if for continue running, when prev == next
    ...
    {
		// KevinBoos: this code does not actually make a difference
		//if (dom0) {
		//	if ((prev->domain->domain_id != DOMID_IDLE) && (prev->domain->domain_id != 0)) {
		//		for (int i = 0; i < 4; i++) {
		//			if (dom0->shared_info->native.sched_infos[i].domid == prev->domain->domain_id) {
		//				dom0->shared_info->native.sched_infos[i].runstate = RUNSTATE_running;
		//				// what else should we set here for a task that continues running?

		//				printk("KevinBoos: prev==next updating domid=%hu continue running, tslice=%ld!!\n", 
		//						next->domain->domain_id, next_slice.time);
		//			}
		//		}
		//	}
		//}
